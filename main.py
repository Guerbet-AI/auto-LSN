...
def compute_LSN(image_list,liver_segmentation_list, OUTPUT_path,
    ...
    for path_nifti_image, path_nifti_liver_segmentation in zip(image_list, liver_segmentation_list):
        ...
        with open(join(OUTPUT_path, filename_id), 'wb') as f:
            pickle.dump(RESULTS, f)




compute_LSN(image_list[5:7], liver_segmentation_list[5:7], OUTPUT_path,
            PERCENTAGE_CENTRAL_SLICES,
            SAVGOL_DEGREE,SAVGOL_WINDOW_LENGTH_IN_MM,UPSAMPLE_RATIO, 
            CONTRAST_THRESHOLD, RADIUS_EXCLUSION_NEIGHBOURHOOD_CONTRAST_IN_MM, 
            R_DISK_IN_MM, R_MASK_IN_MM,WIDTH_IN_MM)
...
results_list_path = glob.glob(os.path.join(OUTPUT_path, f'results_LSN_score_for_*_contrast_{CONTRAST_THRESHOLD}_exclusion_{RADIUS_EXCLUSION_NEIGHBOURHOOD_CONTRAST_IN_MM}_R_DISK_IN_MM_{R_DISK_IN_MM}_R_MASK_IN_MM_{R_MASK_IN_MM}_savgol_{SAVGOL_WINDOW_LENGTH_IN_MM}_degree{SAVGOL_DEGREE}.pkl'))
...
for result in results_list:
    ...
    results = pickle.load(open(join(OUTPUT_path, result), 'rb'))
...
df.to_excel(join(OUTPUT_path, 'df_contrast_%d_exclusion_%d_R_DISK_IN_MM_%d_R_MASK_IN_MM_%s_savgol_%ddegree_%d.xlsx'% (
        CONTRAST_THRESHOLD,RADIUS_EXCLUSION_NEIGHBOURHOOD_CONTRAST_IN_MM,
